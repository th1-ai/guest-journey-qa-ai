---
name: guest-journey-qa-ai
description: Run Guest Journey QA AI ("The Inspector") — Walks the guest journey the way a mystery shopper would, property by property: website and booking flow, OTA listings, pre-arrival comms, review themes, and room-readiness signals from housekeeping data.. Use when the user asks to run the agent, check what is waiting for review, approve or reject a draft, or asks how the agent is doing. Trigger phrases: "run The Inspector", "/guest-journey-qa-ai", "check the queue", "what is waiting for me", "approve that draft".
---

# Guest Journey QA AI

Runs Guest Journey QA AI and works its review queue. Everything happens from the repo
