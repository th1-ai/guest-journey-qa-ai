...
You are working inside **Guest Journey QA AI** ("The Inspector") — Walks the guest journey the way a mystery shopper would, property by property: website and booking flow, OTA listings, pre-arrival comms, review themes, and room-readiness signals from housekeeping data..
...
## Agent specifics: Guest Journey QA AI

**Main workflow:** `workflows/10-qa-audit.md`. One weekly pass
(`python3 tools/run.py --once`) scores every property in
`config/agent.yaml: properties` from five CSV imports in `data/imports/`,
and queues one GM digest per property. Everything is deterministic
arithmetic (`tools/signals.py` + `tools/scorecard.py`) - there is no
classify/draft LLM step like most agents in this family. The only model
call anywhere in this repo is the optional, off-by-default portfolio note
(`prompts/portfolio-note.md`) - see `docs/how-it-works.md` "The one LLM
call" before assuming a pending prompt means something is broken; on
`llm.provider: interactive` that note is skipped entirely, never parked.

**Sub-agents:** none. This repo has no `2x-*.md` workflow and no folded
children.

**What needs a human:** every scorecard, always - `mode: shadow` means
nothing is ever sent without approval, same as every agent in this family.
A property is additionally flagged `needs_human` (rather than plain
`pending_review`) when it has no `gm_email` configured, or when none of the
five signal CSVs covered it this week - both reasons are printed on the
scorecard's own `warnings` list, so read those before deciding what to do.

**The one thing worth remembering about this agent's shape:** unlike Hello
Desk (the reference agent this factory was built from), there is no
classify-then-draft pipeline and no `always_needs_human` intent list.
Read `docs/how-it-works.md` before assuming the reference agent's patterns
apply directly - the review FSM, the shadow guard and the CLI shapes are
identical; the reasoning pipeline is not.
