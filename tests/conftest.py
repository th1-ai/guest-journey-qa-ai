...
done in this working copy. `AGENT_REPO_ROOT` is sandboxed too, so a test can
never read a hotel's real `data/imports/*.csv`.
"""
...
def isolated_settings(tmp_path, monkeypatch):
    ...
    def _make(**kwargs):
        ...
        # and prompts/knowledge/fixtures from repo_root(), and a hotel's own
        # files must never reach a test.
...
@pytest.fixture
def signals_dir():
    return REPO_ROOT / "fixtures" / "inbound" / "signals"
